...
describe('useFoodStore', () => {
	it('starts with exactly the built-in foods', () => {
		...
		expect(result.current.foods).toEqual(builtInFoods)
	})
	...
	it('assigns distinct sequential ids when addFood is called multiple times', () => {
		const { result } = renderHook(() => useFoodStore(), { wrapper })

		let first: ReturnType<typeof result.current.addFood> | undefined
		let second: ReturnType<typeof result.current.addFood> | undefined
		act(() => {
			first = result.current.addFood({ name: 'A', price: 1, description: 'a', tags: [] })
			second = result.current.addFood({ name: 'B', price: 2, description: 'b', tags: [] })
		})

		expect(first?.id).not.toBe(second?.id)
		expect(second?.id).toBe((first?.id ?? 0) + 1)
		expect(result.current.foods).toHaveLength(builtInFoods.length + 2)
	})
